...
sleep 5
echo "[*] Constellation Node 1 Ready"
nohup constellation-node tm2.conf 2>> qdata/logs/constellation2.log &
sleep 5
echo "[*] Constellation Node 2 Ready"
nohup constellation-node tm3.conf 2>> qdata/logs/constellation3.log &
sleep 5
echo "[*] Constellation Node 3 Ready"
nohup constellation-node tm4.conf 2>> qdata/logs/constellation4.log &
sleep 5
echo "[*] Constellation Node 4 Ready"
nohup constellation-node tm5.conf 2>> qdata/logs/constellation5.log &
sleep 5
echo "[*] Constellation Node 5 Ready"

sleep 1

echo "[*] Starting node 1 - Bank 1"
PRIVATE_CONFIG=tm1.conf nohup geth --rpccorsdomain "*" --datadir qdata/dd1 $GLOBAL_ARGS --raftport 50401 --rpcport 22000 --port 21000 2>>qdata/logs/1.log &

echo "[*] Starting node 2 - Bank 2"
PRIVATE_CONFIG=tm2.conf nohup geth --rpccorsdomain "*" --datadir qdata/dd2 $GLOBAL_ARGS --raftport 50402 --rpcport 22001 --port 21001 2>>qdata/logs/2.log &

echo "[*] Starting node 3 - Bank 3"
PRIVATE_CONFIG=tm3.conf nohup geth --rpccorsdomain "*" --datadir qdata/dd3 $GLOBAL_ARGS --raftport 50403 --rpcport 22002 --port 21002 2>>qdata/logs/3.log &
...
PRIVATE_CONFIG=tm5.conf nohup geth --rpccorsdomain "*" --datadir qdata/dd5 $GLOBAL_ARGS --raftport 50405 --rpcport 22004 --port 21004 2>>qdata/logs/5.log &
